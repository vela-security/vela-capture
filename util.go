...
func NotListenPort(proto string) lua.Slice {
	var of *ss2.OptionFlag
	if proto == "tcp" {
		of, _ = ss2.NewOptionFlag("-l -t") //LISTEN TCP IPv4 or IPv6
	} else {
		of, _ = ss2.NewOptionFlag("-l -u") //LISTEN UDP IPv4 or IPv6
	}

	ln := ss2.By(ss2.Flag(of)) // or not(host 192.168.1.1 and dst port 8080)

	if ln == nil || len(ln.Sockets) == 0 {
		return nil
	}

	f := func(sock *ss2.Socket) string {
...
func NotListenBPF(proto string) lua.Slice {
	var of *ss2.OptionFlag
	if proto == "tcp" {
		of, _ = ss2.NewOptionFlag("-l -t")
	} else {
		of, _ = ss2.NewOptionFlag("-l -u")
	}
